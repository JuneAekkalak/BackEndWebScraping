...
const scraper = async () => {
  try {
    ...
    for (let i = roundScraping; i < totalAuthors; i += batchSize) {
      ...
      const scrapingPromises = batchAuthors.map(async (author) => {
        ...
        try {
          const authorData = Object.keys(author_backup).length === 0
            ? await scrapeAuthorData(author.url, page)
            : author_backup;

          const article = await scrapeArticleData(author.url, page);

          if (Object.keys(authorData).length !== 0 && article.length !== 0) {
            authorData.articles = article;
